...
async function recalculate() {
    ...
    const correctStandings = players.map((p) => {
        const pid = p.id;
    ...
    const pointsMapOld = new Map();
    const opponentsMapOld = new Map();
    for (const p of players) {
        pointsMapOld.set(p.id, 0);
        opponentsMapOld.set(p.id, []);
    }
    ...
    const oldStandings = players.map((p) => {
        const pid = p.id;
