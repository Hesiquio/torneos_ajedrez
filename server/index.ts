...
import { db } from './db';
import { generateRoundRobin, generateCatchUpMatches } from './scheduler';
  ...
  try {
    ...
    if (status === 'in_progress') {
      ...
      if (existingPlayerIds.length > 0) {
        ...
        const startRoundNum = ((roundsRes.rows[0] as any).max_round || 0) + 1;

        // Generate catch-up rounds
        const doubleRound = type === 'double';
        const catchUpRounds = generateCatchUpMatches(playerId, existingPlayerIds, doubleRound, startRoundNum);
