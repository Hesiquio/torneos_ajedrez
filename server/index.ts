...
import { db } from './db';
import { generateRoundRobin, assignMidTournamentMatches } from './scheduler';
...
app.get('/api/tournaments/:id', async (req, res) => {
  ...
  try {
    ...
    matches.forEach((m: any) => {
      // Ignore bye matches
      if (m.is_bye || m.black_player_id === 'BYE') return;
...
app.post('/api/tournaments/:id/players', async (req, res) => {
  ...
  try {
    ...
    const { status, type } = tResult.rows[0] as any;
    ...
    if (status === 'in_progress') {
      ...
      const existingPlayersRes = await db.execute({
        sql: 'SELECT id FROM players WHERE tournament_id = ? AND id != ?',
        args: [id, playerId]
      });
      const existingPlayerIds = existingPlayersRes.rows.map((r: any) => r.id);

      if (existingPlayerIds.length > 0) {
        // Find existing rounds to fill
        const roundsData = await db.execute({
          sql: `
        ...
        const roundsMap = new Map<string, { id: string; roundNumber: number; busyPlayerIds: Set<string> }>();
        roundsData.rows.forEach((row: any) => {
          ...
          if (row.white_player_id) rd.busyPlayerIds.add(row.white_player_id);
          if (row.black_player_id && row.black_player_id !== 'BYE') rd.busyPlayerIds.add(row.black_player_id);
        });
        ...
        const { roundAssignments, newRoundsNeeded } = assignMidTournamentMatches(
          playerId,
          existingPlayerIds,
          type === 'double',
          existingRounds,
        ...
        for (const m of roundAssignments) {
          await db.execute({
            ...
            args: [randomUUID(), id, m.roundId, m.whitePlayerId, m.blackPlayerId]
          });
        }

        // Regenerate BYE matches for anyone missing a match in any round
        const allRealMatches = await db.execute({ sql: "SELECT round_id, white_player_id, black_player_id FROM matches WHERE tournament_id = ?", args: [id] });
        const allPlayers = await db.execute({ sql: "SELECT id FROM players WHERE tournament_id = ?", args: [id] });
        const allRounds = await db.execute({ sql: "SELECT id FROM rounds WHERE tournament_id = ?", args: [id] });

        const playerIds = allPlayers.rows.map((r: any) => r.id);
        const matchesByRound = new Map<string, Set<string>>();
        allRounds.rows.forEach((r: any) => matchesByRound.set(r.id, new Set()));
        allRealMatches.rows.forEach((m: any) => {
            ...
            for (const pId of playerIds) {
                if (!busyPlayers.has(pId)) {
                    await db.execute({
                        ...
                        args: [randomUUID(), id, roundId, pId, 'BYE']
                    });
                }
            }
        }
...
app.post('/api/tournaments/:id/start', async (req, res) => {
  ...
  try {
    ...
    const players = playersRes.rows.map((r: any) => ({ id: r.id, name: r.name }));
...
app.post('/api/tournaments/:id/compress-rounds', async (req, res) => {
  ...
  try {
    ...
    const allRounds = roundsRes.rows.map((r: any) => ({ id: r.id, roundNumber: r.round_number, busyPlayerIds: new Set<string>(), matchIds: [] as string[] }));

    // 3. Separate completed vs pending matches
    const completedMatches = allMatches.filter((m: any) => m.result && m.result !== 'pending');
    const pendingMatches = allMatches.filter((m: any) => !m.result || m.result === 'pending');

    // 4. Populate busy players from completed matches
    const roundMap = new Map<string, typeof allRounds[0]>();
    ...
    const updates: { matchId: string, newRoundId: string }[] = [];

    pendingMatches.forEach((m: any) => {
      let assignedRoundId: string | null = null;
      ...
      if (!assignedRoundId) {
        ...
        const newRound = { id: newRoundId, roundNumber: nextRoundNum++, busyPlayerIds: new Set<string>([m.white_player_id, m.black_player_id]), matchIds: [m.id] };
    ...
    for (const r of roundsToDelete) {
      await db.execute({ sql: 'DELETE FROM rounds WHERE id = ?', args: [r.id] });
    }
    ...
    for (let i = 0; i < finalRounds.length; i++) {
      ...
      if (finalRounds[i].roundNumber !== correctNum) {
        await db.execute({ sql: 'UPDATE rounds SET round_number = ? WHERE id = ?', args: [correctNum, finalRounds[i].id] });
        finalRounds[i].roundNumber = correctNum;
      }
    }

    // 11. Regenerate BYE matches for final rounds
    const playersRes = await db.execute({ sql: 'SELECT id FROM players WHERE tournament_id = ?', args: [id] });
    const playerIds = playersRes.rows.map((p: any) => p.id);
    ...
    for (const r of finalRounds) {
      for (const pId of playerIds) {
        if (!r.busyPlayerIds.has(pId)) {
          await db.execute({
            sql: 'INSERT INTO matches (id, tournament_id, round_id, white_player_id, black_player_id, is_bye) VALUES (?, ?, ?, ?, ?, 1)',
            args: [randomUUID(), id, r.id, pId, 'BYE']
          });
        }
      }
    }
...
app.post('/api/matches/:id/result', async (req, res) => {
  ...
  try {
    ...
    const tMatchesRes = await db.execute({
    ...
    const tStats = tMatchesRes.rows[0] as any;
...
app.patch('/api/tournaments/:id/archive', async (req, res) => {
  ...
  try {
    ...
    const tRes = await db.execute({ sql: 'SELECT status, prev_status FROM tournaments WHERE id = ?', args: [id] });
    if (tRes.rows.length === 0) return res.status(404).json({ error: 'Tournament not found' });
    const current = tRes.rows[0] as any;

    if (unarchive) {
      const restoreStatus = current.prev_status || 'in_progress';
      await db.execute({ sql: 'UPDATE tournaments SET status = ?, prev_status = NULL WHERE id = ?', args: [restoreStatus, id] });
    } else {
      await db.execute({ sql: "UPDATE tournaments SET prev_status = status, status = 'archived' WHERE id = ?", args: [id] });
    }
    res.json({ success: true });
  } catch (error) {
...
async function runMigrations() {
  try {
    await db.execute('ALTER TABLE tournaments ADD COLUMN prev_status TEXT');
    console.log('Migration: added prev_status column to tournaments');
  } catch {}
  try {
    await db.execute('ALTER TABLE matches ADD COLUMN is_bye INTEGER DEFAULT 0');
